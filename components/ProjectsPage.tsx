  ...
  useEffect(() => {
      try {
          ...
          if (stored) {
              ...
              // Filter out drafts
              setAllProjects(projects.filter((p: any) => p.status === 'Published'));
          } else {
              // If empty, seed with defaults and show them
              localStorage.setItem('site_projects', JSON.stringify(defaultProjects));
      ...
      if (savedConfig) {
          try {
              ...
              if (parsed.heroAvatar) setAvatar(parsed.heroAvatar);
