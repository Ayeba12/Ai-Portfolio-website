...
const defaultProjects = [
  {
    ...
    longDescription: "A sovereign AI legal analyst for a Tier-1 firm. The system autonomously reviews NDAs and contracts, flagging risk with 99.4% accuracy against senior partner benchmarks.",
    year: "2024",
  ...
  },
  {
    ...
    date: "Aug 2024",
    subtitle: "Predictive logistics engine for global shipping",
    tags: ["Machine Learning", "Automation", "ERP"],
    image: "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?q=80&w=1000&auto=format&fit=crop",
    longDescription: "Integrated predictive ML models into a legacy logistics ERP. Resulted in a 40% reduction in shipping delays by autonomously rerouting cargo based on weather and port congestion data.",
    year: "2024",
  ...
  },
  {
    ...
    longDescription: "Engineered a HIPAA-compliant voice agent for patient intake. Handles 5,000+ simultaneous calls, reducing wait times from 45 minutes to <10 seconds.",
    year: "2024",
  ...
  },
  {
    ...
    date: "May 2024",
    subtitle: "Self-evolving component library",
    tags: ["GenUI", "React", "System Design"],
    image: "https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=1000&auto=format&fit=crop",
    longDescription: "Created a self-evolving component library for a fintech super-app. The UI automatically adjusts density and contrast based on user trading velocity and ambient lighting.",
    year: "2024",
  ...
  return (
    ...
    >
      <div
        ...
        aria-hidden="true"
      />
      <div className="relative w-full max-w-6xl max-h-[90vh] bg-background border border-border rounded-xl shadow-2xl flex flex-col md:flex-row overflow-hidden animate-in zoom-in-95 duration-300">

        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 rounded-full bg-background/95 text-primary hover:bg-accent hover:text-black border border-transparent transition-all"
...
export const Portfolio: React.FC<PortfolioProps> = ({ onNavigate, onOpenCaseStudy }) => {
  ...
  const loadProjects = async () => {
    try {
      ...
      console.error("Failed to load projects in Portfolio", e);
      setAllProjects(defaultProjects);
    }
  };
        ...
        <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-6 transition-all duration-700 delay-200 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          ...
          {filteredProjects.slice(0, activeFilter === 'All' ? 4 : undefined).map((project, index) => {
            ...
            return (
              <div
                ...
                className={`${spanClass} group relative flex flex-col justify-between rounded-xl overflow-hidden bg-surface border border-border cursor-pointer hover:border-accent/50 transition-all duration-500 min-h-[400px] lg:min-h-[450px]`}
              >
                {/* Text Content Area */}
                <div className="p-8 relative z-20">
                  ...
                  <h3 className="text-3xl font-display font-medium text-primary mb-3 group-hover:text-accent transition-colors leading-tight">
                    {project.title}
                  </h3>
                  <p className="text-secondary text-sm leading-relaxed line-clamp-2 max-w-sm mb-6">
                    {project.subtitle}
                  </p>

                  <div className="flex flex-wrap gap-2">
                    {project.tags?.map((tag: string) => (
                      <span key={tag} className="px-2 py-0.5 bg-accent/10 border border-accent/20 rounded text-[10px] uppercase tracking-wider font-bold text-accent">
                        {tag}
                      </span>
                    ))}
                  </div>
                ...
                {/* Hover Action Icon */}
                <div className="absolute top-8 right-8 w-10 h-10 bg-background/80 backdrop-blur-md rounded-full flex items-center justify-center text-primary opacity-0 group-hover:opacity-100 transform translate-y-2 group-hover:translate-y-0 transition-all duration-300 border border-border">
          ...
          {activeFilter === 'All' && (
            <div
              ...
              className="lg:col-span-4 md:col-span-2 relative group rounded-xl overflow-hidden bg-accent cursor-pointer flex flex-col items-center justify-center p-8 min-h-[400px] lg:min-h-[450px] transition-transform duration-500 hover:-translate-y-1"
            >
